...
def type_weighted_anomaly_torch_channels(pred: torch.Tensor, target: torch.Tensor, metric_type="all") -> torch.Tensor:
    ...
    if metric_type == "all":
        ...
        result_nume = torch.mean(weight * (pred - torch.mean(weight * pred, dim=(-1, -2), keepdim=True)) * (target - torch.mean(weight * target, dim=(-1, -2), keepdim=True)))
        ...
        northern_result_nume = torch.mean(northern_weight * (pred[:, :, northern_index:] - torch.mean(northern_weight * pred[:, :, northern_index:], dim=(-1, -2), keepdim=True)) * (target[:, :, northern_index:] - torch.mean(northern_weight * target[:, :, northern_index:], dim=(-1, -2), keepdim=True)))
        ...
        southern_result_nume = torch.mean(southern_weight * (pred[:, :, :souther_index] - torch.mean(southern_weight * pred[:, :, :souther_index], dim=(-1, -2), keepdim=True)) * (target[:, :, :souther_index] - torch.mean(southern_weight * target[:, :, :souther_index], dim=(-1, -2), keepdim=True)))
        ...
        tropics_result_nume = torch.mean(tropics_weight * (pred[:, :, souther_index:northern_index] - torch.mean(tropics_weight * pred[:, :, souther_index:northern_index], dim=(-1, -2), keepdim=True)) * (target[:, :, souther_index:northern_index] - torch.mean(tropics_weight * target[:, :, souther_index:northern_index], dim=(-1, -2), keepdim=True)))
